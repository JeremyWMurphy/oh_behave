...
config.n_start_gomax = 2; % number of trials to put at the begining of max stim go trials to get the animal going
...
config.tp.lickMax = 1; % uint, how many licks before calling it an early lick
...
while f.UserData.state ~= 3
    ...
    if f.UserData.state == 1 % begin a run
        ...
        elseif f.UserData.run_type == 1 || f.UserData.run_type == 2 % detection or pairing
            ...
            if run_type == 1
                fprintf(data_fid_notes,'\nDetection Task');
                write_serial(s,'<P,12,0>'); % don't reward all outcomes
                pause(config.baseln)
            elseif run_type == 2
                fprintf(data_fid_notes,'\nPairing Task');
                write_serial(s,'<P,12,1>'); % reward all outcomes
                pause(config.baseln)
            end
